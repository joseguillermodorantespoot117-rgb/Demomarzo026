...
year_range = st.slider(
    ...
    min_value=min_year,
    max_value=max_year,
    value=(min_year, max_year) # Valor inicial: rango completo
)

# Filtrar datos según el rango seleccionado
filtered_year_counts = year_counts[
    (year_counts['Year'] >= year_range[0]) & 
    (year_counts['Year'] <= year_range[1])
]

fig_year_line = px.line(
    filtered_year_counts, # Usar datos filtrados
    x='Year',
    y='Count',
    title=f'Tendencia de Personajes por Año de Primera Aparición ({year_range[0]}-{year_range[1]})',
